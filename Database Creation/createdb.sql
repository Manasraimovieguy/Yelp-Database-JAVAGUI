CREATE TABLE Users(userId VARCHAR2(50) primary key, name VARCHAR2(50), yelpingSince date,  votesFunny int, votesUseful int, votesCool int, review_count int , fans int, averageStars float, totalvotes int );

CREATE TABLE UserFriends(userId VARCHAR2(50), friend_id VARCHAR2(50) );

CREATE TABLE Reviews(reviewId VARCHAR2(50) primary key , stars number check (stars between 1 and 5), publishDate date , text VARCHAR2(4000) , businessId VARCHAR2(50) , userId VARCHAR2(50),  votesFunny number, votes_cool number, votes_useful number);
